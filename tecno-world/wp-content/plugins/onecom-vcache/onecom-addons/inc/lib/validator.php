...
if( ! function_exists( 'oc_validator_load_textdomain' ) ) {
	function oc_validator_load_textdomain() {
		...
		if(strpos(get_locale(), 'en_') === 0){
			load_textdomain( 'validator', dirname( __FILE__ ) . '/languages/validator-en_US.mo' );
		}
		else{
            load_plugin_textdomain( 'validator', false, trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) . trailingslashit( 'languages' ) );   
        }
	}
}

if (!defined('OC_WPAPI_ENDPOINT')) {
    define('OC_WPAPI_ENDPOINT', 'https://wpapi.one.com/api/v1.0/features');
}
...
if(!function_exists('oc_pm_features')){
    function oc_pm_features($key, $val){
        $operations = [
            'ins' => 'ONE_CLICK_INSTALL',
            'stg' => 'STAGING_ENV',
            'stheme' => 'STANDARD_THEMES',
            'ptheme' => 'PREMIUM_THEMES',
...
if( ! function_exists('oc_get_logline')){
    function oc_get_logline($post_data){
        ...
        $feature_condition = ( isset($post_data['feature']) && ($post_data['feature'] != '') );
        $theme_condition = ( isset($post_data['theme']) && ($post_data['theme'] != '') );
        
        if ( ! ($feature_condition || $theme_condition) ){
            return;
        }
        
        if ( isset($post_data['feature']) && $post_data['feature'] != ''){
            $feature = filter_var($post_data['feature'], FILTER_SANITIZE_STRING);
        ...
        if ( isset($post_data['theme']) && ($post_data['theme'] != '')){
            $theme = filter_var($post_data['theme'], FILTER_SANITIZE_STRING);
            $message .= ";theme:$theme";
        }
        ...
        $message .= ";features_available:$feature_string";        
        return $message;
    }
}
	...
	function oc_validate_action(action){
		...
		return jQuery.ajax({
            ...
            error: function (xhr, textStatus, errorThrown) {
                oc_alert("<?php echo htmlentities(__("Some error occurred, please reload the page and try again.", "validator")); ?>", 'error', 5000)
            }
        });
	}
    </script>
...
if(!function_exists('onecom_upgrade_modal')){
 function onecom_upgrade_modal(){
    ...
    $thm_screens = array('themes', 'one-com_page_onecom-wp-themes');
   ...
   ?>
    <div id="oc_um_overlay" style="display:none;">
        <div id="oc_um_wrapper">
            <div id="oc_um_head">
                <h5><i class="inline_icon" style="margin-right: 8px;background:url('data:image/svg+xml;base64,');height: 13.5px;display: inline-block;vertical-align: middle;background-repeat: no-repeat;width: 9px;"></i> <?php echo __('Premium feature', 'validator'); ?></h5>
            </div>
            <div id="oc_um_body">
                <?php 
                if($type=='themes'): ?>
                    <p><?php echo __('The chosen theme is not available in your current subscription.', 'validator'); ?> </p>
                <?php else: ?>
                    <p><?php echo __('This feature is not available in your current subscription.', 'validator'); ?> </p>
                <?php endif; ?>

                <p><?php echo __('Please upgrade to a subscription with access to Premium features.', 'validator'); ?> <a class="inline_badge_link" style="color: #95265e;font-family: Open Sans;-webkit-font-smoothing: antialiased;font-size: 12px;font-weight: 600;line-height: 18px;cursor: pointer;text-decoration:none;" href="<?php echo onecom_generic_locale_link('premium_page', get_locale()); ?>" target="_blank"><?php echo __('Learn more', 'validator'); ?></a></p>
            </div>

            <div id="oc_um_footer">
                <a href="javascript:;" onclick="jQuery('#oc_um_overlay').hide();jQuery('.loading-overlay.fullscreen-loader').removeClass('show');" class="oc_um_btn oc_cancel_btn"><?php echo __('Cancel', 'validator'); ?></a>
                <a href="https://login.one.com/cp/subscription#targetDomain=<?php echo isset($_SERVER['ONECOM_DOMAIN_NAME']) ? $_SERVER['ONECOM_DOMAIN_NAME']:''; ?>" target="_blank" class="oc_um_btn oc_up_btn"><?php echo __('Upgrade', 'validator'); ?></a>
            </div>
...
add_filter('oc_preview_install', 'oc_inline_badge', 10, 3); // attach with preview install
add_filter('oc_staging_button_create', 'oc_inline_badge',10,3); // attach with staging create
add_filter('oc_staging_button_delete', 'oc_inline_badge',10,3); // attach with staging delete
...
if( ! function_exists( 'onecom_fetch_themes' ) ) {
	function onecom_fetch_themes( $page = 1, $exclude_ilotheme = false) {
		...
		$themes = (array) get_site_transient( 'onecom_themes' );
		...
		if( ! empty( $themes ) && isset($themes['item_count']) && $themes['item_count'] >= 1000) {
		...
		if( is_wp_error( $response ) ) {
			if( isset( $response->errors[ 'http_request_failed' ] ) ) {
				$errorMessage = __( 'Connection timed out', 'validator' );
			} else {
			...
			if( wp_remote_retrieve_response_code( $response ) != 200 ) {
				...
				if( ! empty($body) && $body->success ) {
					...
					if( $body->error == 'RESOURCE NOT FOUND' ) {
						...
						$errorMessage = __( 'Sorry, no compatible themes found for your version of WordPress and PHP.', 'validator' ).'&nbsp;<a href="'.$try_again_url.'">'.__( 'Try again', 'validator' ).'</a>';
					} else {
...
if(!function_exists('oc_check_theme_eligibility')){
    function oc_check_theme_eligibility($features, $stylesheet=''){
        ...
        if(in_array('PREMIUM_THEMES', $features)){
            return true;
        }

        // check if non-premium WP package & trying to use STANDARD THEME
        if(
            (in_array('ONE_CLICK_INSTALL', $features) || in_array('STANDARD_THEMES', $features)) 
            && !onecom_is_premium_theme($theme->display( 'Name', FALSE ))
        ){
            return true;
        }
...
if(!function_exists('onecom_premium_theme_admin_notice')){
    function onecom_premium_theme_admin_notice($html='') {
        $badge = '<div class="oc_notice notice notice-error"><p><i class="inline_icon" style="background:url(\'data:image/svg+xml;base64,\');height: 13.5px;display: inline-block;vertical-align: middle;background-repeat: no-repeat;width: 9px;"></i> &nbsp;'.__('You have chosen a Premium feature that is not included in your current subscription.', 'validator').' '.__('Please upgrade to a subscription with access to Premium features.','validator').' <a class="inline_badge_link" style="margin-left: 5px;color: #95265e;font-family: Open Sans;-webkit-font-smoothing: antialiased;font-size: 13px;font-weight: 600;line-height: 18px;cursor: pointer;text-decoration:none;" href="'.onecom_generic_locale_link('premium_page', get_locale()).'" target="_blank">'.__('Learn more', 'validator').'</a></p></div>';
        if($html != ''){
            return $html . $badge;
        }
        echo $badge;
...
if(!function_exists('oc_check_theme_before_switch')){
    function oc_check_theme_before_switch(){
        ...
        if( !(isset($_GET['enable']) || isset($_GET['action'])) && !(isset($_GET['theme']) || isset($_GET['stylesheet']))){
            return;
        }
        ...
        if(! (isset($_GET) && !empty($_GET) && ($current_page=='themes' || $current_page=='themes-network'))){
            return;
        }

        // check if oc_error
        if( isset( $_GET['error'] ) && 'oc_error' == $_GET['error'] ) {
        ...
        if(!(isset($_GET['theme']) || isset($_GET['stylesheet']))){
            return;
        }

        // get stylesheet name for single/network site
        $stylesheet = isset($_GET['theme']) ? $_GET['theme'] : $_GET['stylesheet'];
        ...
        if ( false === strpos( $referer, '/network/themes.php' ) ) {
            ...
            wp_safe_redirect( add_query_arg( 'error', 'oc_error', $referer ) );
        }
        exit;
    }
}
    ...
    function oc_pm_badge_injection($hook_suffix)
    {
        ...
        foreach($installed_themes as $theme){
            if( 'one.com' === strtolower( $theme->display( 'Author', FALSE ) ) 
                ...
                $themes_to_mark_array[] = str_replace(' ', '-', strtolower($theme->display( 'Name', FALSE )));
...
add_action( 'customize_controls_enqueue_scripts', function () {
    ...
    foreach($installed_themes as $theme){
        if( 'one.com' === strtolower( $theme->display( 'Author', FALSE ) ) 
            ...
            $themes_to_mark_arr[] = strtolower($theme->display( 'Name', FALSE ));
...
if( ! function_exists( 'onecom_get_client_ip_env' ) ) {
	function onecom_get_client_ip_env() {
	    ...
	    return $_SERVER['ONECOM_CLIENT_IP'];
	}
}
